...
class NLPProcessor:
    ...
    def generate_response(
        self, user_message: str, history: List[Tuple[str, str]] = None
    ) -> str:
        """
        ...
        system_prompt = (
            ...
            "- Если перечисляешь несколько советов или пунктов, разделяй их пустой строкой (двойным переносом строки).\n"
            "- Каждый новый пункт начинай с цифры и точки (1., 2., ...) и пиши с новой строки.\n"
            "Не используй звёздочки для списков.\n"
            ...
            "Вместо этого предложи конкретные техники самопомощи."
            "ТЫ — ПСИХОЛОГ. Если пользователь задаёт вопрос не о психологии (например, о домашнем задании, математике, программировании, бытовых вещах), "
            "ТЫ НЕ ДОЛЖЕН НА НЕГО ОТВЕЧАТЬ. Вместо этого вежливо скажи, что твоя специализация — психологическая поддержка, и предложи поговорить о чувствах или стрессовых ситуациях. "
            "Пример ответа: «Я психолог и могу помочь тебе с эмоциональными состояниями. Расскажи, что тебя беспокоит или тревожит?»"
        )
        ...
        output = self.llm(
            ...
            stop=[
                "</s>",
                "\nПользователь:",
            ],  # останавливаемся на конце ответа или начале новой реплики
            echo=False,
        )
